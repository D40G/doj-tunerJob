...
RegisterNetEvent("renzu_stancer:addstancer", function(vehicle) -- Standalone Purpose
	AddStancerKit(vehicle)
end)

RegisterNetEvent("renzu_stancer:addstancerForTuner", function(vehicle)-- doj-tunerJob
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
	AddStancerKit(vehicle)
  Player.Functions.RemoveItem("stancerkit", 1)
  TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['stancerkit'], "remove", 1)
  TriggerClientEvent('QBCore:Notify', src, "Camber arms have been installed", 'success')
end)

RegisterNetEvent("renzu_stancer:airsuspension", function(entity,val,coords)
