...
function GetPlayerFromId(src)
	...
	elseif Config.Framework == 'QBCORE' then
		...
		selfcore.data.removeInventoryItem = function(item,val)
			TriggerClientEvent('QBCore:Notify', tonumber(self.src), "Camber arms have been applied", 'success')
			QBCore.Functions.GetPlayer(tonumber(self.src)).Functions.RemoveItem(item, val)
			TriggerClientEvent('inventory:client:ItemBox', tonumber(self.src), QBCore.Shared.Items[item], "remove", val)
		end
